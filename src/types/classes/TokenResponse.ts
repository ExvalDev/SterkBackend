...
export default class TokenResponse {
  ...
  constructor(message: string, access_token: string, refresh_token: string) {
    ...
    this.access_token_expires = this.calculateTokenExpiry(ACCESS_TOKEN_LIFE);
    this.refresh_token = refresh_token;
    this.refresh_token_expires = this.calculateTokenExpiry(REFRESH_TOKEN_LIFE);
  }

  calculateTokenExpiry = (tokenLife: string) => {
